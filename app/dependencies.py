...
async def get_current_active_user(
    ...
    (Note: get_current_user already raises 401 if inactive per requirements,
    but this dependency explicitly asserts it as well)
    """
...
async def get_current_user(
    ...
    if await redis_client.get(f"banned_user:{user_id}"):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
